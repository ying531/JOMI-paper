...
# =============================================================================
LEN_MAX = 5
selected_test_idx = np.arange(0, 2000)[itv_len <= LEN_MAX]
...
if len(selected_test_idx) > 0:
    ...
    if_in_q1 = 1 * (calib_score_pred <= eta_neg) * (calib_len <= LEN_MAX) + 1 * (calib_score_pred > eta_neg) * (calib_len_neg <= LEN_MAX)
    if_in_q2 = 1 * (calib_score_pred <= hat_eta) * (calib_len_pos <= LEN_MAX) + 1 * (calib_score_pred > hat_eta) * (calib_len <= LEN_MAX)
...
results.to_csv(SAVE_PATH+"/length_seed_"+str(seed)+"_alpha_"+str(q_ind)+".csv")   
